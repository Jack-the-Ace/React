...
import paris from './assets/thumbs-up.png'

//리액트에서 화면의 구성요소가 되려면.. Component라는 클래스를 상속해야 함.
class MyApp extends Component{   // 상속을 통해 Component의 능력(멤버들)을 가지게 됨

    // MyApp 컴포넌트가 보여줄 화면을 그리는 기능함수(이름이 정해져있음)
    render(){
        // 브라우저가 보여줄 글씨를 리턴해주면 표시됨
        //return "Hello My Webapp"

        // 태그문 글씨를 출력해보면? -- 따옴표를 사용하면 그냥 문자열데이터임, 태그문인식못함 --
        // return "<h2>Hello</h2>"

        //따옴표 없이 태그문을 사용하면 React가 알아서 DOM객체로 만들어 화면을 구성함
        ...
        return (
            // class명 지정할때.. 속성명 class 대신 className 사용을 권장- why? JS에서 class는 설계도를 만드는 키워드
            <div className='wrap'>
                ...
                {/* 이미지 보여주기(별도수업예정) - 맛보기 */}
                {/* src폴더에서 작업하는 component의 이미지들은 src폴더 안에 위치하는것을 선호함 */}
                {/* 이미지를 사용하려면 import해야 사용 가능함. */}
                <img src={paris} alt="thumbs-up image" />
            </div>
        )
...
// 다른 문서(main.jsx)에서 MyApp 를 사용하려면.. export 해야함.
