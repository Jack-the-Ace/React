...
class Home extends Component{
    ...
    // 컴포넌트 A가 보여줄 글씨 데이터(값이 변경되면 자동으로 화면갱신)
    state={
        'nickname':'oops',
    }
    // 컴포넌트 B가 클릭됐을때 실행할 함수
    ...
    render(){
        return(
            <>
                ...
                {/* 2) 다른 문서에서도 재사용하게 하고싶다면 별도의 .jsx로 화면조각을 만드는 것이 좋음 */}
                <MyComponent2></MyComponent2>
                <MyComponent2/>

                {/* 3) 컴포넌트를 재사용할때 매번 같은 글씨만 보이는 것 효용성이 없어보임. */}
                   {/* 그래서 컴포넌트를 재사용할때 데이터를 속성 property 으로 전달해보기 */}
                <MyComponent3 aaa='sam' age='20' color='red'></MyComponent3>
                <MyComponent3 aaa='robin' age='25' color='green' />
                {/* 혹시 속성들을 안주면? */}
