...
class HomeComponent extends Component{
    ...
    render(){
        ...
        return(
            <div>
                ...
                {/* 1. 버튼 클릭 이벤트 처리 (html때와 약간 다름) - 함수의 이름만 써야함.. 호출하면 안됨. 마치 함수를 등록해놓은 느낌 */}
    ...
    increaseAge=()=>{
        //state값의 변경은 반드시 setState()만 해야함
    ...
    changeName3= ()=>{
        // state 변수의 값 변경을 무조건 setState()라는 함수를 이용해야만 갱신됨
        this.setState({name:'hong'})
    }


    // react 에서 이벤트 처리용 함수는 무조건 화살표함수로...
    changeName2= ()=>{
        this.name='robin';
        // 변수를 바꿨다고 해서 화면을 다시 갱신해 주지 않음.
        // 화면을 다시 그리라고 명령.
        this.forceUpdate()
        // 이렇게 갱신하라고 명령하는 것 짜증.. 알아서 갱신되었으면 한다면..
        // 값이 변경되었을때 자동으로 화면이 갱신되는 아주 특별한 변수 state 를 활용
    }

    // 이름 변경 버튼 클릭했을때 실행될 멤버함수
    changeName= function(){
        // 기존 방식과 완전 다른 방식으로 요소의 글씨를 변경.
        // 요소가 변수의 값을 보여주고.. 클릭되었을때,. 요소를 찾지않고.. 변수값만 변경하면
        // 그 변수를 보여주는 요소의 글씨가 변경되도록 하는 기법(data binding 이라고 부름)
        this.name= 'robin'
        // 이유?> 익명함수도 객체가 될 수 있기에, this.name을 HomeComponent말고
        // 본인changeName함수객체의 멤버인줄 오해함.
        // 그래서.. this.키워드의 영향을 받지 않으려면..
        // 화살표 함수를 써야 함. 화살표함수는 객체가 되지 않는 함수임.
    }

    // es6 에서 도입된 또 다른 방법 : 화살표함수(react에서 권장!!!!)
    ddd=()=>{
        alert('화살표 함수')
    }
