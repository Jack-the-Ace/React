...
function TodoInput(props){  //전달받은 값/함수들 받는 파라미터

    //input요소에 써있는 글씨를 저장할 state변수
    const [content, setContent]= useState('')

    // 버튼 클릭할때 발동할 함수
    const clickAddBtn=()=>{
        // input요소에 써있는 글씨를 가져와서.. props 전달받은 insertTodo()함수를 실행시켜 항목 추가
        props.insertTodo(content)
        // 다음 입력이 편하도록.. input 요소의 value 값을 빈글씨로..
        setContent('')
    }
    ...
    const submit=()=>{
        ...
        if(content=='') return  //글씨 쓴 것이 없으면 동작 종료
