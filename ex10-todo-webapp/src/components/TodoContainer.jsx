...
function TodoContainer(){
    ...
    const deleteTodo=(no)=>{
        // 기존 state 배열에서 요소만 삭제하면 setState()할때 배열객체의 변화가 없다고
        //생각하여 화면갱신이 자동으로 되지 않음.

        // 리액트팀에서 배열의 요소를 삭제하는 권장기법(.filter기능함수 사용)
        const newTodos=todos.filter((todo,idx)=>{return todo.no!=no})  // no빼고 나머지만 살아남아라..
        setTodos(newTodos)
    }

    // todos 항목들의 index번째 todo 객체의 isDone 값을 toggle..
    const toggleDone=(index)=>{
        // 원래 todos 배열의 요소값을 바꾸어도 화면은 새로 갱신되지 않음.
        // 원래 배열을 복제한 완전 새로운 배열 만들기
        const newTodos=[ ...todos ]  // ... 스프레드연산자(todos의 요소들을 전개시켜줌)
