...
function Home(){
    ...
    const submitGet= (event)=>{
        //form요소는 action속성이 있든없든.. 무조건 페이지를 갱신함. SPA에 맞지않음
        //그래서 기본기능을 막기
    ...
    const changeCheckbox= (value)=>{
        if(fruits.includes(value)){
            //배열요소의 제거는.. 그 요소만 제외하고 필터링된 새 배열 만들어야 함.
            const newFruits= fruits.filter(element=>element!=value)
            setFruits(newFruits)
        }else{
            //배열요소의 추가도.. 원래 배열을 복제한 새 배열을 만들고 value 추가.
    ...
    // 사용자가 input요소의 파일선탯을 변경했을때 실행될 함수
    const changeFile= (event)=>{
        //다른 input 요소와 다르게 value로 값을 얻으면.. 가짜 경로가 옴.
        //alert(event.target.value)

        //실제 파일데이터를 가져와야함
        const files= event.target.files  //여러개 선탯일 수 있어서 배열로 옴

        // 이 예제는 파일 1개짜리... [파일명, 파일사이즈, 파일타입] 확인
        const filename= files[0].name
        const filesize= files[0].name
    ...
    // 다중선택이 가능한 파일선택이 완료되면...
    const [imgUrls, setImgUrls]= useState([])
    const changeFiles= (event)=>{
        // 선택한 파일들의 정보를 가져오기
        const files= event.target.files

        // 파일 정보를 가진 객체를 그대로 img 요소에 표시될 수 없음.
        // URL정보로 변경해야 함.
        const newImageUrls= []
        for(const file of files){
        ...
        // 이 이미지의 url을 state 변수에 저장하여 화면이 새로 그려지도록..
    ...
    return (
        <div>
            ...
            {/* why? 버튼클릭말고. 키보드 enter 키를 눌렀을때도 전송되도록 하고싶다면.. form 사용 */}
